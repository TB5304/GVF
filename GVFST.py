...
#         st.write(f'Normal depth (yn) = {yn} m')
        # Display results
        st.write('## Results:')
        st.markdown(f'* Reservoir water level (Y<sub>0</sub>) = **{Y0:.2f} m**')
        st.markdown(f'* Critical height (y<sub>c</sub>) = **{yc:.2f} m**')
        st.markdown(f'* Unit discharge from reservoir (q) = **{q:.2f} m<sup>2</sup>/s**')
        st.markdown(f'* Discharge (Q) = **{Q:.2f} m<sup>3</sup>/s**')
        st.markdown(f'* Flow area (A) = **{A:.2f} m<sup>2</sup>**')
        st.markdown(f'* Wetted perimeter (P) = **{P:.2f} m**')
        st.markdown(f'* Hydraulic radius (R) = **{R:.2f} m**')
        st.markdown(f'* Critical slope (S<sub>C</sub>) = **{SC:.4f}**')
        st.markdown(f'* GVF profile = **{GVF_profile}**')
        st.markdown(f'* Water profile = **{water_profile}**')
        st.markdown(f'* Normal depth (y<sub>n</sub>) = **{yn:.2f} m**')
...
Y0 = st.sidebar.number_input('Enter Reservoir Water Level/Depth of flow Y0 (m)',format="%.3f", min_value=0.0, max_value=10.0, value=6.0, step=0.1)
B = st.sidebar.number_input('Enter bottom width B (m)',format="%.3f", min_value=0.0, max_value=10.0, value=10.0, step=0.1)
n = st.sidebar.number_input('Enter Manning roughness coefficient n',format="%.3f", min_value=0.01, max_value=0.1, value=0.013, step=0.01)
S0 = st.sidebar.number_input('Enter channel bed slope S0', format="%.3f", min_value=0.0, max_value=0.1, value=0.01, step=0.001) 
#value = st.number_input("Enter a value:", step=0.0001)
